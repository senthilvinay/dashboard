...
def _simulate_discover(jid: str, clusters: list) -> dict:
    ...
    for cluster in clusters:
        ...
        for dep in DEP_NAMES:
            pods = [
                {
                    "name":     f"{dep}-{hex(random.randint(0x10000, 0xfffff))[2:]}",
            ...
            _emit(jid, f"  {dep}: {len(pods)} pod(s)  [{running} Running]", "#a0b4c8")
        result[cluster] = deps
    return result
        ...
        try:
            ...
            for cluster in clusters:
                ...
                if HAS_SSH:
                    ...
                    pods = [
                        {
                            "name":        f"pnsrt-input-{hex(random.randint(0x10000,0xfffff))[2:]}",
                            "status":      "Running",
                            "ready":       "1/1",
