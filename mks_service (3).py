...
def _setup_env(ssh, jid: str, user: str, ns: str):
    """
    Set up environment on jump server.
    Uses ABSOLUTE PATHS from config — does NOT use 'module load'
    because module system requires ksh and fails under paramiko bash sessions.
    kubectl and unimatrix are called directly via their full firm paths.
    """
    cfg = get_config()

    # Source profile to get basic PATH/ENV
    for cmd in [
    ...
    KUBECTL   = cfg.get("kubectl_path",
                        "/ms/dist/cloud/PROJ/kubectl/prod/bin/kubectl")
    UNIMATRIX = cfg.get("unimatrix_path",
                        "/ms/dist/cloud/PROJ/unimatrix/prod/bin/unimatrix")

    # Verify the binaries exist on the jump server
    for binary, name in [(KUBECTL, "kubectl"), (UNIMATRIX, "unimatrix")]:
        _, _, rc = _run(ssh, f"test -x {binary} && echo OK || echo MISSING", 10)
        out, _, _ = _run(ssh, f"test -x {binary} && echo OK || echo MISSING", 10)
        status = "✓" if "OK" in out else "⚠"
        col    = "#00e676" if "OK" in out else "#ffab00"
        _emit(jid, f"{status} {name}: {binary}", col)

    # Set KUBECONFIG and NAMESPACE
    _run(ssh, f"export KUBECONFIG=/var/tmp/{user}/.kube/config 2>/dev/null; true", 10)
...
def _simulate_access(jid: str, user: str, server: str, clusters: list):
    ...
    steps = [
        ...
        (0.3,  f"$ test -x {KUBECTL}",                       "#00e5ff"),
        (0.3,  "✓ kubectl binary found",                     "#00e676"),
        (0.3,  f"$ test -x {UNIMATRIX}",                     "#00e5ff"),
        (0.3,  "✓ unimatrix binary found",                   "#00e676"),
    ]
...
def _simulate_discover(jid: str, clusters: list) -> dict:
    ...
    cfg = get_config()
    # Deployments come ONLY from config file — never hardcoded here.
    # In real mode (paramiko installed), this function is never called —
    # kubectl get deployments fetches the live list from the cluster.
    DEP_NAMES = cfg.get("deployments", [])
    if not DEP_NAMES:
        _emit(jid, "⚠ Simulation: no deployments configured.", "#ffab00")
        _emit(jid, "  Add 'deployments' list to pnsrt_mks_restart_config.json,", "#ffab00")
        _emit(jid, "  OR install paramiko for live kubectl fetch: pip install paramiko", "#ffab00")
        ...
        try:
            if HAS_SSH:
                ...
                for cluster in clusters:
                    ...
                    UNIMATRIX2 = UNIMATRIX.replace("/unimatrix/", "/unimatrixv2/")
                    # Try unimatrixv2 first; fall back to unimatrix activate
                    roles_ok = 0
                    roles_fail = 0
                    for role in ["application-deployment", "monitor-with-logs", "monitor"]:
                        cmd = (
                            f"{UNIMATRIX2} activations create "
                            f"--cluster {cluster} --namespace {ns} "
                            f"--role {role} --justification {tcm}"
                        )
                        _emit(jid, f"$ {cmd}", "#00e5ff")
                        out2, err2, rc = _run(ssh, cmd, 90)
                        if rc == 0:
                            _emit(jid, f"  ✓ {role}", "#00e676")
                            roles_ok += 1
                        else:
                            # Log full error so user can see what went wrong
                            detail = (out2 or err2 or "no output")[:120]
                            _emit(jid, f"  ✗ {role}: {detail}", "#ff5252")
                            roles_fail += 1

                    if roles_fail > 0 and roles_ok == 0:
                        _emit(jid,
                            f"  ⚠ All role activations failed for {short}. "
                            f"Check TCM {tcm} is valid and namespace {ns} exists.",
                            "#ffab00")
            ...
            job["status"] = "DONE"
            _emit(jid, "\n✅ MKS Access step complete — proceed to Step 2", "#00e676")
            _emit(jid, "   Note: even if role activations failed, SSH + unimatrix login", "#a0b4c8")
            _emit(jid, "   succeeded. Kubectl operations may still work via existing kubeconfig.", "#a0b4c8")

        except Exception as ex:
            ex_type = type(ex).__name__
            # Detect SSH auth failure explicitly
            if HAS_SSH and 'AuthenticationException' in ex_type:
                job["status"] = "FAILED"
                job["error"]  = "SSH authentication failed — wrong username or password"
                _emit(jid, "✗ SSH Authentication Failed", "#ff5252")
                _emit(jid, "  Check your username and password and try again.", "#ff5252")
            elif HAS_SSH and 'NoValidConnectionsError' in ex_type:
                job["status"] = "FAILED"
                job["error"]  = f"Cannot connect to {server} — host unreachable"
                _emit(jid, f"✗ Cannot connect to {server}", "#ff5252")
            elif HAS_SSH and 'SSHException' in ex_type:
                job["status"] = "FAILED"
                job["error"]  = f"SSH error: {ex}"
                _emit(jid, f"✗ SSH error: {ex}", "#ff5252")
            else:
                job["status"] = "FAILED"
                job["error"]  = str(ex)
                ...
                for cluster in clusters:
                    ...
                    try:
                        for item in json.loads(dep_out).get("items", []):
                            ...
                            # Get pods using the deployment's own selector labels
                            # (more reliable than guessing label name)
                            selector_labels = {}
                            try:
                                selector_labels = item["spec"]["selector"].get("matchLabels", {})
                            except Exception:
                                pass
                            if selector_labels:
                                sel_str = ",".join(f"{k}={v}" for k,v in selector_labels.items())
                            else:
                                sel_str = f"app={dep_name}"  # fallback
                            pod_out, _, _ = _run(
                                ssh,
                                f"{KUBECTL} get pods -l '{sel_str}' -o json -n {ns}",
                                30,
                            )
                            pods = []
                            try:
                ...
                for cluster in clusters:
                    ...
                    if mode == "all":
                        if HAS_SSH:
                            ...
                            # Simulation: read from config, never hardcode
                            deps = get_config().get("deployments", [])
                            if not deps:
                                _emit(jid, "⚠ No deployments in config — add 'deployments' list or install paramiko for live fetch", "#ffab00")
